...
@Service
public class ConsejosServicioImpl implements ConsejosServicio {

    @Override
    public RespuestaConsejo evaluar(SolicitudConsejo solicitud) {
        String topic = solicitud.topic().trim();
        String key = topic.toLowerCase(Locale.ROOT);

        if (matchesUrgent(key)) {
            return new RespuestaConsejo(
                    topic,
        ...
        if (matchesModerate(key)) {
            ...
            return new RespuestaConsejo(topic, message, "amarillo", "Síntomas moderados — consulta recomendada");
        }
        ...
        return new RespuestaConsejo(topic, message, "verde", "Información general / síntomas leves");
    }
